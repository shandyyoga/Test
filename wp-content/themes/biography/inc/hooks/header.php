...
function biography_header() {
    ...
    $biography_logo = $biography_customizer_saved_options['biography-logo'];
    }
    else{
        $biography_logo = '';
    }
    $biography_header_bg_img =  get_header_image() ;
    $biography_enable_social_icons = $biography_customizer_saved_options['biography-enable-social-icons'];
    ...
    ?>
    <header id="masthead" class="site-header block-section biography-nav-left" role="banner" style="background-image: url('<?php echo  esc_url($biography_header_bg_img);?>');">
                                ...
                                if( !empty( $biography_logo)){?>
                                    <div class="photo-section">
                                      ...
                                      <?php  if ( !function_exists( 'has_custom_logo' ) ) { ?>
                                            <img src="<?php echo esc_url($biography_logo);?>" alt="<?php esc_attr( get_bloginfo( 'name' ) ); ?>">
                                      <?php } else { ?>
 ...
 */
    function biography_add_breadcrumb(){
        ...
        global $biography_customizer_saved_options;
      $biography_enable_breadcrumb = $biography_customizer_saved_options['biography-enable-breadcrumb'];
        // Bail if Home Page
        if ( is_front_page() || is_home() || 1 != $biography_enable_breadcrumb ) {
            return;
        }
