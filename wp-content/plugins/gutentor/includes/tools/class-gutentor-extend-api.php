...
if ( ! class_exists( 'Gutentor_Extend_Api' ) ) {
	...
	class Gutentor_Extend_Api {
         ...
         * @return string
         */
		public function new_badge_product( $post, $product ) {
			...
			if ( ( time() - ( 60 * 60 * 24 * $newness_days ) ) < $created ) {
				return apply_filters( 'gutentor_woocommerce_new_badge', '<span class="gutentor-wc-new">' . esc_html__( 'New!', 'gutentor' ) . '</span>', $post, $product );
		...
		public function add_product_data( $data, $post, $request ) {
			...
			$data['product_new_badge']       = $this->new_badge_product( $post, $product );
