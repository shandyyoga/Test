...
if ( ! class_exists( 'Gutentor_P6' ) ) {
	...
	class Gutentor_P6 extends Gutentor_Block_Base {
		...
		public function load_dependencies() {
			...
			require_once GUTENTOR_PATH . 'includes/block-templates/p6/template1.php';
			require_once GUTENTOR_PATH . 'includes/block-templates/p6/template2.php';
		}
		...
		public function render_callback( $attributes, $content ) {
			...
			if ( $the_query->have_posts() ) :
				$output .= '<' . $tag . ' class="' . apply_filters( 'gutentor_post_module_main_wrap_class', gutentor_concat_space('section-' . $gID,'gutentor-post-module','gutentor-post-module-p6', $template, $align ), $attributes ) . '" id="' . esc_attr( $blockID ) . '" data-gbid="' . esc_attr( $gID ) . '" ' . GutentorAnimationOptionsDataAttr( $blockComponentAnimation ) . '>' . "\n";
